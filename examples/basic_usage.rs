...
use array_range_query::{
    LazySegTree, LazySegTreeAddSum, LazySegTreeSpec, SegTree, SegTreeMax, SegTreeMin, SegTreeSpec,
    SegTreeSum,
};
...
fn custom_lazy_example() {
    ...
    println!("Initial sum [0, 5): {}", lazy_tree.query(0, 5)); // 15
    ...
    println!("Sum [0, 5): {}", lazy_tree.query(0, 5)); // 1 + 12 + 13 + 14 + 5 = 45

    // Add 5 to range [0, 2)
    lazy_tree.update(0, 2, 5);
    println!("\nAfter adding 5 to range [0, 2):");
    println!("Sum [0, 2): {}", lazy_tree.query(0, 2)); // (1+5) + (12+5) = 23
    println!("Sum [0, 5): {}", lazy_tree.query(0, 5)); // 6 + 17 + 13 + 14 + 5 = 55
    println!();
}
...
fn lazy_helper_example() {
    ...
    println!("Initial values: {:?}", values);
    println!("Sum [0, 5): {}", tree.query(0, 5)); // 30
    println!("Sum [1, 4): {}", tree.query(1, 4)); // 4 + 6 + 8 = 18

    // Add 3 to range [1, 4)
    tree.update(1, 4, 3);
    println!("\nAfter adding 3 to range [1, 4):");
    println!("Sum [0, 5): {}", tree.query(0, 5)); // 2 + 7 + 9 + 11 + 10 = 39
    println!("Sum [1, 4): {}", tree.query(1, 4)); // 7 + 9 + 11 = 27

    // Add -2 to range [0, 3)
    tree.update(0, 3, -2);
    println!("\nAfter subtracting 2 from range [0, 3):");
    println!("Sum [0, 5): {}", tree.query(0, 5)); // 0 + 5 + 7 + 11 + 10 = 33
    println!("Sum [0, 3): {}", tree.query(0, 3)); // 0 + 5 + 7 = 12
    println!();
}
...
fn advanced_lazy_example() {
    ...
    println!("Initial: 8 ones, sum = {}", tree.query(0, 8));

    // Perform overlapping updates
    tree.update(0, 4, 2); // Add 2 to first half
    tree.update(2, 6, 3); // Add 3 to middle section (overlaps)
    tree.update(4, 8, 1); // Add 1 to second half

    println!("After overlapping updates:");
    println!("Range [0, 2): sum = {}", tree.query(0, 2)); // (1+2) + (1+2) = 6
    println!("Range [2, 4): sum = {}", tree.query(2, 4)); // (1+2+3) + (1+2+3) = 12
    println!("Range [4, 6): sum = {}", tree.query(4, 6)); // (1+3+1) + (1+3+1) = 10
    println!("Range [6, 8): sum = {}", tree.query(6, 8)); // (1+1) + (1+1) = 4
    println!("Total sum: {}", tree.query(0, 8)); // 6 + 12 + 10 + 4 = 32
