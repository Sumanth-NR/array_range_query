...
///
/// assert_eq!(tree.query(0, 5), 5); // Maximum of all elements
///
/// // Add 10 to range [1, 4)
/// tree.update(1, 4, 10);
/// assert_eq!(tree.query(0, 5), 15); // 1, max(13, 12, 15), 4 = 15
/// ```
...
mod tests {
    ...
    fn test_add_max_basic_operations() {
        ...
        assert_eq!(tree.query(0, 5), 5); // Max of all: max(1,3,2,5,4) = 5
    ...
    fn test_add_max_range_updates() {
        ...
        assert_eq!(tree.query(0, 5), 50); // max(10, 25, 35, 45, 50) = 50
        assert_eq!(tree.query(1, 4), 45); // max(25, 35, 45) = 45
        assert_eq!(tree.query(0, 2), 25); // max(10, 25) = 25

        // Add 20 to range [0, 3)
        tree.update(0, 3, 20);
        // Values become: [30, 45, 55, 45, 50]
        assert_eq!(tree.query(0, 3), 55); // max(30, 45, 55) = 55
        assert_eq!(tree.query(0, 5), 55); // max(30, 45, 55, 45, 50) = 55
    }
    ...
    fn test_add_max_overlapping_updates() {
        ...
        assert_eq!(tree.query(0, 5), 8); // max(3,4,8,6,5) = 8
    }
    ...
    fn test_add_max_negative_updates() {
        ...
        assert_eq!(tree.query(0, 5), 50); // max(10, 15, 25, 35, 50) = 50
        assert_eq!(tree.query(1, 4), 35); // max(15, 25, 35) = 35

        // Mix positive and negative updates
        tree.update(0, 2, -15); // Subtract 15 from first two
                                // Values become: [-5, 0, 25, 35, 50]
        assert_eq!(tree.query(0, 2), 0); // max(-5, 0) = 0
        assert_eq!(tree.query(0, 5), 50); // max(-5, 0, 25, 35, 50) = 50
    }
    ...
    fn test_add_max_edge_cases() {
        ...
        let mut tree = LazySegTreeAddMax::<i32>::from_vec(&values);
        let original_max = tree.query(0, 5);
        tree.update(2, 2, 100); // Empty range update
        assert_eq!(tree.query(0, 5), original_max); // Should be unchanged
    }
    ...
    fn test_add_max_large_tree() {
        ...
        assert_eq!(tree.query(0, size), size as i64); // Maximum is 1000

        // Add 1000 to first half
        tree.update(0, size / 2, 1000);
        // First half becomes: 1001 to 1500, second half remains: 501 to 1000
        assert_eq!(tree.query(0, size), 1500); // New maximum
        assert_eq!(tree.query(0, size / 2), 1500); // First half max
        assert_eq!(tree.query(size / 2, size), 1000); // Second half max
    }
    ...
    fn test_add_max_zero_updates() {
        ...
        let original_max = tree.query(0, 4);

        // Adding zero should not change anything
        tree.update(0, 4, 0);
        assert_eq!(tree.query(0, 4), original_max);
    ...
    fn test_add_max_all_negative() {
        ...
        assert_eq!(tree.query(0, 5), -3); // max(-10, -5, -15, -3, -8) = -3

        // Add positive value to make some elements positive
        tree.update(1, 4, 10);
        // Values become: [-10, 5, -5, 7, -8]
        assert_eq!(tree.query(0, 5), 7); // max(-10, 5, -5, 7, -8) = 7
