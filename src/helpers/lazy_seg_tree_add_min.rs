...
/// // Add 2 to range [1, 4)
/// tree.update(1, 4, 2);
/// assert_eq!(tree.query(0, 6), 3); // min(5, 4, 10, 3, 9, 3)
/// ```
...
mod tests {
    ...
    fn test_add_min_basic_operations() {
        ...
        assert_eq!(tree.query(0, 6), 1); // min(5,2,8,1,9,3) = 1
    ...
    fn test_add_min_range_updates() {
        ...
        // Add 5 to range [1, 4)
        tree.update(1, 4, 5);
        assert_eq!(tree.query(0, 5), 10); // min(10,25,35,45,50) = 10
        assert_eq!(tree.query(1, 4), 25); // min(25,35,45) = 25

        // Add -15 to range [0, 3)
        tree.update(0, 3, -15);
        assert_eq!(tree.query(0, 5), -5); // min(-5,10,20,45,50)
        assert_eq!(tree.query(0, 3), -5); // min(-5,10,20)
    }
    ...
    fn test_add_min_negative_updates() {
        ...
        tree.update(1, 4, -5);
        assert_eq!(tree.query(0, 5), 10); // min(10,15,25,35,50) = 10
        assert_eq!(tree.query(1, 4), 15); // min(15,25,35) = 15

        // Mix positive and negative updates
        tree.update(0, 2, 15); // [25,30,25,35,50]
        assert_eq!(tree.query(0, 2), 25); // min(25,30)
        assert_eq!(tree.query(0, 5), 25); // min(25,30,25,35,50)
    }
    ...
    fn test_add_min_edge_cases() {
        ...
        let mut tree = LazySegTreeAddMin::<i32>::from_vec(&values);
        let original_min = tree.query(0, 5);
        tree.update(2, 2, 100); // Empty range update
        assert_eq!(tree.query(0, 5), original_min); // Should be unchanged
    }
    ...
    fn test_add_min_large_tree() {
        ...
        assert_eq!(tree.query(0, size), 1); // Second half is still 1

        // Subtract 2 from second half
        tree.update(size / 2, size, -2);
        assert_eq!(tree.query(0, size), -1); // Now min is -1
        assert_eq!(tree.query(size / 2, size), -1);
    }
