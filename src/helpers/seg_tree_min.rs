...
mod tests {
    ...
    fn test_min_basic_operations() {
        ...
        assert_eq!(tree.query(0, 6), 1); // Min of all: min(5,2,8,1,9,3) = 1
        assert_eq!(tree.query(1, 4), 1); // Min of middle: min(2,8,1) = 1
        assert_eq!(tree.query(0, 1), 5); // Single element
    ...
    fn test_min_updates() {
        ...
        tree.update(0, 100);
        assert_eq!(tree.query(0, 5), 20); // min(100,20,30,40,50) = 20
        assert_eq!(tree.query(0, 1), 100); // Just the updated element
        assert_eq!(tree.query(1, 5), 20); // min(20,30,40,50) = 20

        // Update middle element to smallest value
        tree.update(2, 5);
        assert_eq!(tree.query(0, 5), 5); // min(100,20,5,40,50) = 5
        assert_eq!(tree.query(2, 3), 5); // Just the updated element
        assert_eq!(tree.query(0, 3), 5); // min(100,20,5) = 5
    }
    ...
    fn test_min_edge_cases() {
        ...
        assert_eq!(tree_single.query(0, 0), i32::MAX);

        // All same values
        let same = vec![7, 7, 7, 7, 7];
        let tree_same = SegTreeMin::<i32>::from_vec(&same);
        assert_eq!(tree_same.query(0, 5), 7);
        ...
        assert_eq!(tree.query(0, 0), i32::MAX); // Empty range at start
        assert_eq!(tree.query(8, 8), i32::MAX); // Empty range at end
    }
    ...
    fn test_min_large_tree() {
        ...
        assert_eq!(tree.query(0, size), 1);

        // Minimum of first half (1 to 500) is 1
        assert_eq!(tree.query(0, 500), 1);

        // Minimum of second half (501 to 1000) is 501
        assert_eq!(tree.query(500, size), 501);

        // Test update - change the minimum
        tree.update(0, 2000); // Change 1 to 2000
        assert_eq!(tree.query(0, size), 2); // New minimum is 2
        assert_eq!(tree.query(0, 500), 2); // First half minimum is now 2
        assert_eq!(tree.query(0, 1), 2000); // Just the updated element
    }
    ...
    fn test_min_negative_values() {
        ...
        assert_eq!(tree.query(0, 5), -5); // min(-5, -3, -1, 2, 4) = -5
        assert_eq!(tree.query(0, 3), -5); // min(-5, -3, -1) = -5
        assert_eq!(tree.query(3, 5), 2); // min(2, 4) = 2
        assert_eq!(tree.query(1, 4), -3); // min(-3, -1, 2) = -3

        tree.update(0, -10); // Change -5 to -10
        assert_eq!(tree.query(0, 5), -10); // New minimum is -10
        assert_eq!(tree.query(1, 5), -3); // Excluding first element
    }
    ...
    fn test_min_new_empty_tree() {
        ...
        assert_eq!(tree.query(0, 5), i32::MAX);
        ...
        assert_eq!(tree.query(0, 5), 10); // min(MAX, 10, MAX, 20, MAX) = 10
        assert_eq!(tree.query(1, 4), 10); // min(10, MAX, 20) = 10
        assert_eq!(tree.query(3, 5), 20); // min(20, MAX) = 20
        assert_eq!(tree.query(0, 1), i32::MAX); // Still MAX
    }
    ...
    fn test_min_extremes() {
        ...
        assert_eq!(tree.query(0, 5), i32::MIN); // MIN is smallest
        assert_eq!(tree.query(1, 5), -1); // Excluding MIN: min(MAX, 0, -1, 1) = -1
        assert_eq!(tree.query(1, 2), i32::MAX); // Just MAX

        tree.update(0, 0); // Change MIN to 0
        assert_eq!(tree.query(0, 5), -1); // min(0, MAX, 0, -1, 1) = -1
    }
}
