...
mod tests {
    ...
    fn test_max_basic_operations() {
        ...
        assert_eq!(tree.query(0, 6), 9); // Max of all: max(5,2,8,1,9,3) = 9
    ...
    fn test_max_updates() {
        ...
        tree.update(4, 5);
        assert_eq!(tree.query(0, 5), 40); // max(10,20,30,40,5) = 40
        assert_eq!(tree.query(4, 5), 5); // Just the updated element
        assert_eq!(tree.query(0, 4), 40); // max(10,20,30,40) = 40

        // Update middle element to largest value
        tree.update(2, 100);
        assert_eq!(tree.query(0, 5), 100); // max(10,20,100,40,5) = 100
        assert_eq!(tree.query(2, 3), 100); // Just the updated element
        assert_eq!(tree.query(0, 3), 100); // max(10,20,100) = 100
    }
    ...
    fn test_max_edge_cases() {
        ...
        assert_eq!(tree_single.query(0, 0), i32::MIN);
        ...
        assert_eq!(tree.query(0, 0), i32::MIN); // Empty range at start
        assert_eq!(tree.query(8, 8), i32::MIN); // Empty range at end
    }
    ...
    fn test_max_large_tree() {
        ...
        assert_eq!(tree.query(0, size), 1000);

        // Maximum of first half (1 to 500) is 500
        assert_eq!(tree.query(0, 500), 500);

        // Maximum of second half (501 to 1000) is 1000
        assert_eq!(tree.query(500, size), 1000);

        // Test update - change the maximum
        tree.update(999, 0); // Change 1000 to 0
        assert_eq!(tree.query(0, size), 999); // New maximum is 999
        assert_eq!(tree.query(500, size), 999); // Second half maximum is now 999
    ...
    fn test_max_negative_values() {
        ...
        assert_eq!(tree.query(0, 5), 4); // max(-5, -3, -1, 2, 4) = 4
        assert_eq!(tree.query(0, 3), -1); // max(-5, -3, -1) = -1
        assert_eq!(tree.query(3, 5), 4); // max(2, 4) = 4
        assert_eq!(tree.query(1, 4), 2); // max(-3, -1, 2) = 2

        tree.update(4, -10); // Change 4 to -10
        assert_eq!(tree.query(0, 5), 2); // New maximum is 2
        assert_eq!(tree.query(0, 4), 2); // Excluding last element
    }
    ...
    fn test_max_new_empty_tree() {
        ...
        assert_eq!(tree.query(0, 5), i32::MIN);
        ...
        assert_eq!(tree.query(0, 5), 20); // max(MIN, 10, MIN, 20, MIN) = 20
        assert_eq!(tree.query(1, 4), 20); // max(10, MIN, 20) = 20
        assert_eq!(tree.query(0, 2), 10); // max(MIN, 10) = 10
    ...
    fn test_max_extremes() {
        ...
        assert_eq!(tree.query(0, 5), i32::MAX); // MAX is largest
        assert_eq!(tree.query(0, 1), i32::MIN); // Just MIN
        assert_eq!(tree.query(2, 5), 1); // Excluding MIN and MAX: max(0, -1, 1) = 1

        tree.update(1, 0); // Change MAX to 0
        assert_eq!(tree.query(0, 5), 1); // max(MIN, 0, 0, -1, 1) = 1
    }
}
